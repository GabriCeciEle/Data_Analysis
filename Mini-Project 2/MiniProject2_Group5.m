...
%subplot (in pdf document i think it'll be too small!)
figure('name','different methods X and Y')
subplot(1,2,1)
bar(methodsX,[Results.X.PCAandRegression.trainErr_opt Results.X.PCAandRegression.validationErr_opt;
...
subplot(1,2,2)
bar(methodsY,[Results.Y.PCAandRegression.trainErr_opt Results.Y.PCAandRegression.validationErr_opt;
...
%single bar plot
figure('name','different methods PosX')
bar(methodsX,[Results.X.PCAandRegression.trainErr_opt Results.X.PCAandRegression.validationErr_opt;
    Results.X.PCAandRegression.trainErr_optSecond Results.X.PCAandRegression.validationErr_optSecond;
    Results.X.ElasticNets.trainErr Results.X.ElasticNets.validationErr]);
ylabel('MSE','fontsize',20)
...
figure('name','different methods PosY')
bar(methodsY,[Results.Y.PCAandRegression.trainErr_opt Results.Y.PCAandRegression.validationErr_opt;
    Results.Y.PCAandRegression.trainErr_optSecond Results.Y.PCAandRegression.validationErr_optSecond;
    Results.Y.ElasticNets.trainErr Results.Y.ElasticNets.validationErr]);
ylabel('MSE','fontsize',20)
...
subplot(2,2,[3,4])
plot(xtest(1000:1050),ytest(1000:1050),'linewidth',2)
hold on
plot(Xpredicted_final(1000:1050),Ypredicted_final(1000:1050),'linewidth',2)
title('Real Trajectories samples 1000 to 1050','fontsize',20)
